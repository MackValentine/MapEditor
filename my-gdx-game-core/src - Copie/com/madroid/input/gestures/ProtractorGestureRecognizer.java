...
public class ProtractorGestureRecognizer {
	...
	public void addGestureFromFile(FileHandle handle) {
		if (handle.isDirectory()) {
			...
			JsonValue obj = jreader.parse(handle);

			String _name = obj.getString("Name");
			Array<ObjectMap<String, Float>> _points = new Array<ObjectMap<String, Float>>();// (Array<ObjectMap<String,
																							// Float>>)
																							// obj.get("Points");

			JsonValue p = obj.get("Points");

			for (JsonValue i : p.iterator()) {
				float x = i.getFloat("X");
				float y = i.getFloat("Y");

				ObjectMap<String, Float> o = new ObjectMap<String, Float>();
				o.put("X", x);
				o.put("Y", y);
			...
			Array<Float> _vector = new Array<Float>();// (Array<Float>)
														// obj.get("Vector");

			JsonValue v = obj.get("Vector");

			for (JsonValue i : v.iterator()) {
				float value = i.asFloat();
	...
	/*
	 * @SuppressWarnings("unchecked") public void addGestureFromFile(FileHandle
	 * handle) { if (handle.isDirectory()) { FileHandle[] files =
	 * handle.list("json"); for (FileHandle f : files) addGestureFromFile(f);
	 * 
	 * } else { JsonReader jreader = new JsonReader(); ObjectMap obj =
	 * jreader.parse(handle);
	 * 
	 * String _name = (String) obj.get("Name"); Array<ObjectMap<String, Float>>
	 * _points = (Array<ObjectMap<String, Float>>) obj .get("Points");
	 * Array<Float> _vector = (Array<Float>) obj.get("Vector");
	 * 
	 * ArrayList<Vector2> _arrlist_vector = new ArrayList<Vector2>(); for (int i
	 * = 0; i < _points.size; i++) { ObjectMap<String, Float> data =
	 * _points.get(i); float x = (float) data.get("X"); float y = (float)
	 * data.get("Y"); _arrlist_vector.add(new Vector2(x, y)); }
	 * 
	 * float[] _arr_vector = new float[_vector.size]; for (int i = 0; i <
	 * _vector.size; i++) _arr_vector[i] = _vector.get(i);
	 * 
	 * addGesture(new TemplateGesture(_name, _arrlist_vector, _arr_vector)); } }
	 */
